...
cvar_t	*sv_demofolder;	// the folder that contains server-side demos
...
void SV_Frame( int msec ) {
	...
	while ( sv.timeResidual >= frameMsec ) {
        ...
        // Sync spectator follow target into entity state so cgame scoreboard can display it.
        // The QVM sets ps->pm_flags |= PMF_FOLLOW and ps->clientNum = followed client.
        // We copy that clientNum into ent->s.clientNum so all clients see it in snapshots.
        {
            int si;
            for (si = 0; si < sv_maxclients->integer; si++) {
                client_t *scl = &svs.clients[si];
                if (scl->state != CS_ACTIVE) continue;
                playerState_t *sps = SV_GameClientNum(si);
                if ((sps->pm_flags & 4096) && sps->persistant[PERS_TEAM] == TEAM_SPECTATOR) {
                    sharedEntity_t *sent = SV_GentityNum(si);
                    if (sps->clientNum >= 0 && sps->clientNum < sv_maxclients->integer) {
                        sent->s.clientNum = sps->clientNum;
                    }
                }
            }
        }
