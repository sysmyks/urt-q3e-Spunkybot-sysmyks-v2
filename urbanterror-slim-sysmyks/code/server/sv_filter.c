		...
		else
		{
			...
			if ( node->is_date )
			{
				if ( node->fop == FOP_LT ) // do not print default action for dates
					s = "";
				n = snprintf( buf, sizeof( buf ), "date %s\"%s\"", s, node->p2.string );
			}
			else
			{
				...
				{
					if ( node->is_quoted )
						n = snprintf( buf, sizeof( buf ), "%s %s\"%s\"", node->p1, s, node->p2.string );
					else
						n = snprintf( buf, sizeof( buf ), "%s %s%s", node->p1, s, node->p2.string );
				}
				else
				{
					n = snprintf( buf, sizeof( buf ), "%s %s%i", node->p1, s, node->p2.integer );
				}
			}
