...
#include "server.h"
courseDetection_t currentCourse[MAX_CLIENTS];
...
//	Cvar_Set( "nextmap", va("map %s", server) );
	...
	for (i = 0; i < sv_maxclients->integer; i++) {
		...
		Com_Memset(&svs.clients[i].cm, 0, sizeof(clientMove_t));
	}
	Com_Memset(currentCourse, 0, sizeof(currentCourse));
	...
	// make sure that level time is not zero, and is past the QVM's
	// 30-second vote restriction threshold (level.time - level.lastVoteTime < 30000)
	// so players can vote immediately after a map change via callvote
	if ( sv.time < 30100 ) {
		sv.time = 30100;
	}
...
void SV_Init( void )
{
	...
	sv_demofolder = Cvar_Get ("sv_demofolder", "svdemos", CVAR_ARCHIVE);
