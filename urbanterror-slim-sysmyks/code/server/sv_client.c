...
#define STAMINA_ENABLED  1  // infiniteStamina active
#define STAMINA_DISABLED 2  // infiniteStamina disabled
/*
=================
SV_CreateChallenge
...
int SV_GetClientTeam(int clientNum) {
    ...
    // Guard against calls during map load before gameClients is initialised
    if (!sv.gameClients) {
        return TEAM_SPECTATOR;
    }
		...
		if ( seqs[i] != svs.clients[i].reliableSequence ) {
			for ( n = seqs[i]; n != svs.clients[i].reliableSequence + 1; n++ ) {
				...
				if ( str && str[11] == '\0' && str < cmd + 512 ) {
					int remaining = (int)(cmd + MAX_STRING_CHARS - str);
					if ( *tld == '\0' )
						snprintf( str, remaining, S_COLOR_WHITE "connected (%s)\n\"", country );
					else
						snprintf( str, remaining, S_COLOR_WHITE "connected (" S_COLOR_RED "%s" S_COLOR_WHITE ", %s)\n\"", tld, country );
					break;
				}
			}
		}
...
void SV_DropClient( client_t *drop, const char *reason ) {
	...
	// Stop any server-side demo recording before freeing the client
	if ( drop->demo_recording ) {
		SVD_StopDemoFile( drop );
	}
...
void SV_ClientEnterWorld(client_t *cl, usercmd_t *cmd) {
	...
	if(cmd)
        Com_Memcpy(&cl->lastUsercmd, cmd, sizeof(cl->lastUsercmd));
    else
        Com_Memset(&cl->lastUsercmd, 0, sizeof(cl->lastUsercmd));
...
static void SV_LoadPos_f(client_t *cl) {
	...
	if (isRunning) {
		...
		if (currentTime - cl->cm.lastTeleportTime > 2000) {
			cl->cm.teleportAttempts = 0;
		}

		cl->cm.teleportAttempts++;
		cl->cm.lastTeleportTime = currentTime;
		
		// Ask for confirmation (3 quick presses)
		if (cl->cm.teleportAttempts < 3) {
		...
		if (cid >= 0 && cid < MAX_CLIENTS && currentCourse[cid].detected) {
			Com_Printf("^5Course canceled for %s (manual teleport)\n", cl->name);
			currentCourse[cid].detected = qfalse;
		}
		...
		VM_Call(gvm, 1, GAME_CLIENT_COMMAND, cl - svs.clients);
		cl->cm.teleportAttempts = 0;
		cl->isReady = qfalse; // sync engine state with QVM (two ready toggles = off)
	}
...
static void SV_SavePos_f(client_t *cl)
{
	...
	// Save position in the cm structure (common to both cases)
	VectorCopy(ps->origin, cl->cm.savedPosition);
	VectorCopy(ps->viewangles, cl->cm.savedPositionAngle);
	cl->cm.hasSavedPos = qtrue;
...
static void SV_Noclip_f(client_t *cl) {
	...
	// Toggle noclip directly on playerState - evite la race condition sur sv_cheats global
	if (ps->pm_type == PM_NOCLIP) {
		ps->pm_type = PM_NORMAL;
		ps->persistant[PERS_NOCLIP] = 0;
		SV_SendServerCommand(cl, "print \"^7Noclip ^8disabled\n\"");
		Com_Printf("Noclip disabled for %s\n", cl->name);
	} else {
		ps->pm_type = PM_NOCLIP;
		ps->persistant[PERS_NOCLIP] = 1;
		SV_SendServerCommand(cl, "print \"^7Noclip ^9enabled\n\"");
...
static void SV_Ready_f(client_t *cl) {
    ...
    if (cl->isReady && cl->cm.infiniteStamina == STAMINA_ENABLED) {
        cl->cm.infiniteStamina = STAMINA_DISABLED;
...
qboolean SV_IsClientGhost(client_t *cl) {
    ...
    // Ghost mode is only meaningful in jump gametype (9)
    if (sv_gametype->integer != 9) {
        return qfalse;
    }
    ...
    // If not found or empty, default to off
    if (!ghostValue || !*ghostValue) {
        return qfalse;
    }

    // Only accept strict boolean values to reject garbage input
    if (ghostValue[0] != '0' && ghostValue[0] != '1') {
        return qfalse;
    }

    return (ghostValue[0] == '1');
}
...
void SV_GhostThink(client_t *cl) {
    ...
    if (!cl->cm.ghost) {
        // Ghost disabled: restore self to solid
        if (ent->r.contents & CONTENTS_CORPSE) {
            ent->r.contents &= ~CONTENTS_CORPSE;
            ent->r.contents |= CONTENTS_BODY;
        }
        // Restore any neighbours that may have been made non-solid by this client
        for (i = 0; i < sv_maxclients->integer; i++) {
            client_t *ocl = &svs.clients[i];
            if ((int)(ocl - svs.clients) == cid) continue;
            if (ocl->state != CS_ACTIVE || ocl->cm.ghost) continue;
            oth = SV_GentityNum(i);
            if (oth->r.contents & CONTENTS_CORPSE) {
    ...
    // Ghost enabled: make self non-solid
    if (ent->r.contents & CONTENTS_BODY) {
        ent->r.contents &= ~CONTENTS_BODY;
        ent->r.contents |= CONTENTS_CORPSE;
    }

    // Restore all non-ghost neighbours first (they may have left the radius since last frame)
    for (i = 0; i < sv_maxclients->integer; i++) {
        client_t *ocl = &svs.clients[i];
        if ((int)(ocl - svs.clients) == cid) continue;
        if (ocl->state != CS_ACTIVE || ocl->cm.ghost) continue;
    ...
    // Apply ghost effect to nearby entities to prevent bidirectional collision
    ...
    for (i = 0; i < num; i++) {
        ...
        // Only make non-ghost neighbours temporarily non-solid
        if (svs.clients[touch[i]].cm.ghost) {
            continue;
        }
...
void SV_HelpCmdsList_f(client_t* cl) {
	...
	// Sanitize mapname to prevent quote injection in the print command
	Q_strncpyz(safeMapName, sv_mapname->string, sizeof(safeMapName));
	for (p = safeMapName; *p; p++) {
		if (*p == '"' || *p == '\n' || *p == '\r' || *p == ';') {
			*p = '_';
		}
	}

	// Send help as 3 commands instead of 20+ to avoid reliable-command buffer overflow
	SV_SendServerCommand(cl,
		"print  \""
		"^8----------------------------------------------------------------\n"
		"^8-^7--------> Help Client - ^6List of Commands ^7[Jump Mode] <--------^8-\n"
		"^8----------------------------------------------------------------\n"
		"^8------------------------^7General Commands^8------------------------\n"
		"^8----------------------------------------------------------------\n"
		"^8-    ^1/help           ^8-     ^2/playerlist     ^8-    ^2/cg_rgb        ^8-\n"
		"^8-    ^2/reconnect      ^8-                     ^8-    ^2/r_gamma       ^8-\n"
		"^8-    ^2/disconnect     ^8-     ^1/modinfo      ^8-    ^2/sensitivity   ^8-\n"
		"^8-    ^2/connect        ^8-                     ^8-    ^2/bind          ^8-\n"
		"\"");
	...
	SV_SendServerCommand(cl,
		...
		"^8--------------------------^7Game Commands^8-------------------------\n"
		"^8----------------------------------------------------------------\n"
		"^8-         ^2$location             ^8-       ^2/regainstamina         ^8-\n"
		"^8-         ^2/name                 ^8-       ^2/save                  ^8-\n"
		"^8-         ^2/kill                 ^8-       ^2/load                  ^8-\n"
		"^8-         ^2/ready ^5(RUN)          ^8-       ^2/allowgoto             ^8-\n"
		"^8-         ^2/cg_ghost ^5<0|1>       ^8-       ^2/goto                  ^8-\n"
		"^8----------------------------------------------------------------\n"
		"^8-              ^2!help       ^7[see commands spunky]               ^8-\n"
		"^8-          ^2!register     ^7[register to see all orders]          ^8-\n"
		"^8----------------------------------------------------------------\n"
		"\"");

	SV_SendServerCommand(cl,
		"print  \"^2          [INFO] ^7Current Map:  [^3%s^7]            ^8-\n"
		"^8----------------------------------------------------------------\n"
		"\"",
		safeMapName);
}
...
qboolean SV_ExecuteClientCommand( client_t *cl, const char *s ) {
	...
	for ( ucmd = ucmds; ucmd->name; ucmd++ ) {
		if ( !strcmp( Cmd_Argv(0), ucmd->name ) ) {
			...
			} else if ( ucmd->func == SV_Ready_f ) {
				if ( bFloodProtect ) {
					// Max 2 ready toggles per 3 seconds to prevent QVM flooding
					if ( SVC_RateLimit( &cl->ready_rate, 2, 3000 ) ) {
						return qfalse;
					}
				}
			} else if ( ucmd->func == SV_Noclip_f ) {
				if ( bFloodProtect ) {
					// Max 3 noclip toggles per 2 seconds
					if ( SVC_RateLimit( &cl->ready_rate, 3, 2000 ) ) {
						return qfalse;
					}
				}
...
void SV_ClientThink(client_t *cl, usercmd_t *cmd) {
    ...
    if (isSpectator) {
        ...
        if (cl->isRunning) {
            ...
            int cid_idx = (int)(cl - svs.clients);
            if (cid_idx >= 0 && cid_idx < MAX_CLIENTS && currentCourse[cid_idx].detected) {
                Com_Printf("^5Run interrupted for %s (switched to spectator)\n", cl->name);
                currentCourse[cid_idx].detected = qfalse;
            }
        }
    }
    // Note: ghost state (cl->cm.ghost) and SV_GhostThink are updated once per game frame
    // in sv_main.c to avoid redundant SV_AreaEntities calls per usercmd.
