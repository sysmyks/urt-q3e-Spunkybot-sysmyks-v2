...
typedef struct {
	...
	qboolean hasSavedPos;      // Whether savedPosition contains valid data
	qboolean ghost;
	int infiniteStamina;
	int teleportAttempts;      // /load confirmation counter (reset per map/reconnect)
	int lastTeleportTime;      // svs.time of last /load attempt
} clientMove_t;
...
typedef struct client_s {
	...
	int				demo_backoff;	// how many packets between non-delta frames?
...
extern	cvar_t	*sv_demofolder;	// the folder that contains server-side demos
