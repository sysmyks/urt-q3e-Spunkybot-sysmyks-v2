...
extern courseDetection_t currentCourse[MAX_CLIENTS];
...
void QDECL Com_Printf( const char *fmt, ... ) {
    ...
    static qboolean in_course_detect = qfalse; // guard against recursive calls
    va_list     argptr;
	...
	// in_course_detect prevents a recursive crash: Com_Printf called from within this block
	// would re-enter here and cause infinite recursion / stack overflow.
	if (!in_course_detect && com_sv_running && com_sv_running->integer && svs.clients) {
		in_course_detect = qtrue;
		...
		if (strstr(msg, "started a jump run") || strstr(msg, "ClientJumpRunStarted")) {
			...
			int i;
			const char *p;
			int clientNumFromMsg = -1;

			// Safe: strstr guarantees msg is long enough; compute offset from found pointer
			if ((p = strstr(msg, "ClientJumpRunStarted:")) != NULL)
				clientNumFromMsg = atoi(p + 21); // 21 = strlen("ClientJumpRunStarted:")

			// %31 limits to MAX_NAME_LENGTH-1 to prevent buffer overflow
			if (sscanf(msg, "%31[^:]:", playerName) == 1 ||
				sscanf(msg, "%31[^ ]", playerName) == 1 ||
				sscanf(msg, "broadcast: print \"%31[^ ] started", playerName) == 1) {

				for (i = 0; i < sv_maxclients->integer; i++) {
					client_t *cl = &svs.clients[i];
					if (cl->state >= CS_CONNECTED &&
						(!Q_stricmp(cl->name, playerName) ||
						(clientNumFromMsg >= 0 && i == clientNumFromMsg))) {

						cl->isRunning = qtrue;
						currentCourse[i].detected = qtrue;
						currentCourse[i].startTime = sv.time;
						currentCourse[i].clientNum = i;
						Q_strncpyz(currentCourse[i].playerName, cl->name, sizeof(currentCourse[i].playerName));
		...
		// Patterns to detect the end of a course
			...
			if ((p = strstr(msg, "ClientJumpRunCanceled:")) != NULL)
				clientNumFromMsg = atoi(p + 22); // 22 = strlen("ClientJumpRunCanceled:")
			else if ((p = strstr(msg, "ClientJumpRunStopped:")) != NULL)
				clientNumFromMsg = atoi(p + 21); // 21 = strlen("ClientJumpRunStopped:")

			if (sscanf(msg, "%31[^:]:", playerName) == 1 ||
				sscanf(msg, "%31[^ ]", playerName) == 1 ||
				sscanf(msg, "broadcast: print \"%31[^']", playerName) == 1 ||
				sscanf(msg, "broadcast: print \"Time performed by %31[^:]", playerName) == 1) {
				...
				for (i = 0; i < sv_maxclients->integer; i++) {
					...
					if (cl->state >= CS_CONNECTED &&
						...
						cl->isRunning = qfalse;
						if (currentCourse[i].detected) {
							int runtime = sv.time - currentCourse[i].startTime;
							Com_Printf("^5Course completed for %s (time: %d ms)\n", cl->name, runtime);
							currentCourse[i].detected = qfalse;
						}
						break;
					}
				}
